...
function valuedisplay(value, row, index) {
    ...
    if (value > row.AlertValue * row.DayTime * 6 / 100) {
        return '<div style="width:'+p+'%;height:20px;  line-height: 20px;  position:relative;background:red; "> <span style="text-align:right; width:75px; position:absolute; ">'+a+'</span></div>';
    } else {
        return '<div style="width:' + p + '%;height:20px; line-height: 20px; position:relative;background:green; "> <span style="text-align:right; width:75px; position:absolute; ">' + a + '</span></div>';
    }
}
